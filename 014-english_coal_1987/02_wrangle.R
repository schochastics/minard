...
coal <- read_csv("data/coal.csv") %>% 
  ...
  dplyr::filter(origin=="gbr",dest!="wld",dest!="fdr",year==1987) %>% 
  arrange(-export_val) %>% 
...
# connect iceland 48.383294, -12.417447
# tseq <- seq(0,1,length.out = 10)
# xy_dummy <- rbind(xy_dummy,
#                   matrix(
#                     tseq*c(centroids$x[centroids$iso_a3=="ISL"],centroids$y[centroids$iso_a3=="ISL"]) +
#                       (1-tseq)*c(-12.417447,48.383294),
#                     ncol=2,byrow = TRUE
#                   ))

gtree <- tnss_tree(g,xy,dummy_ocean,root,gamma = 0.85,epsilon = 0.5,elen = 6.5,order = "near")
