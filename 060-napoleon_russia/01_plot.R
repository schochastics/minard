...
# little bit of wrangling and corrections
...
p1 <- ggplot()+
  ...
  scale_y_continuous(limits=c(53.2,56),name="",expand = c(0,0))+
  scale_color_manual(values=c("#D7C181","#424242"))+
  scale_size(range=c(3,24),breaks = c(1e5,2e5,3e5))+
  geom_text_repel(data=Minard.cities,aes(long,lat,label=city),segment.alpha = 0,
                  family = "EB Garamond 08",
                  size=ifelse(Minard.cities$city=="Moscou",10,7),
                  nudge_x = ifelse(Minard.cities$city=="Moscou",0.1,0),
                  nudge_y = ifelse(Minard.cities$city=="Moscou",0.1,0))+
  theme(plot.background = element_rect(fill="#f2eecb",colour=NA),
        panel.background = element_rect(fill="#f2eecb",colour=NA),
...
p2 <- ggplot(Minard.temp,aes(x=long,y=temp,label=paste0(temp,"° ",date)))+
  geom_line()+
  geom_text(family = "EB Garamond 08",size=8,vjust=1,hjust=c(rep(0,8),1))+
  scale_x_continuous(limits=c(24,38),name="")+
  ...
  theme(plot.background = element_rect(fill="#f2eecb",colour=NA),
        ...
        plot.title = element_text(hjust=0.5,size=22,family="EB Garamond 08")
  )+
...
p <- p1+p2+ plot_layout(ncol = 1, heights = c(3, 1))+
  plot_annotation(title=descr,
                  theme = theme(plot.title=element_text(family="EB Garamond 08",size=34,face = "italic"),
                                plot.background = element_rect("#f2eecb")))
h <- 12
w <- h*2.44528
ggsave("060-napoleon_russia.png",p,width=w,height=h,dpi = 300)
